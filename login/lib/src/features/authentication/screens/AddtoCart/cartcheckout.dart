...
class CheckoutPage extends StatelessWidget {
  ...
  CheckoutPage({required this.products, required this.totalPrice});
    ...
    try {
      for (var product in products) {
        order newOrder = order(
          ...
          quantity: product.quantity?.toInt() ?? 0, // Convert double to int
          totalPrice: totalPayment,
          deliveryFee: 200,
          orderDate: DateTime.now(),
          farmerId: product.farmerId ?? '', // Ensure the farmerId is set here
          status: 'Pending',
        );
